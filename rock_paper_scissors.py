import random

def game():
  print("Hello world!")
  # Список вариантов
  options = ["камень", "ножницы", "бумага"]
  
  
  # Счетчик побед пользователя и компа
  user_wins = 0
  computer_wins = 0
  
  print("==============================")
  print("==  Камень Ножницы Бумага  ==")
  print("==============================")
  
  while True:
    
    user_choice = input("\nТвой ход. Напиши камень, ножницы или бумага. Для выхода напиши 0.").lower()
    
    # Проверка на выход
    if user_choice == "0":
      print("===========================================")
      print(f"ИГРА ОКОНЧЕНА!")
      print(f"Ты победил {user_wins} раз.")
      print(f"Компьютер победил {computer_wins} раз.")
      
      if user_wins > computer_wins:
        print(f"Ты победил в общем зачете! Поздравляю!")
      elif computer_wins > user_wins:
        print(f"Ты проиграл в общем зачете. Возможно, в следующий раз повезет больше!")
      else:
        print(f"В общем зачете ничья!")
      print("===========================================")
      print("Спасибо за игру, до свидания!")
      print("===========================================")
      break
    
    # Проверка ввода
    if user_choice not in options:
      print("Ошибка! Напиши именно 'камень', или 'ножницы', или 'бумага'.")
      continue
    
    # Компьютер выбирает
    computer_choice = random.choice(options)
    print(f"Компьютер выбрал: {computer_choice}")
    
    # Выявляем победителя
    # Если ничья
    if user_choice == computer_choice:
      print("Ничья! Играем еще раз.")
    
    # Если победил пользователь
    elif ((user_choice=="камень" and computer_choice == "ножницы") or (user_choice=="ножницы" and computer_choice == "бумага") or (user_choice=="бумага" and computer_choice == "камень")):
      print("Ты победил!")
      user_wins +=1
    
    else:
      print("Победил компьютер!")
      computer_wins +=1
    
    # Выводим счет после каждого раунда.
    print(f"Текущий счет:    Ты: {user_wins}    Компьютер: {computer_wins}")
  
  # Запуск игры
if __name__ == "__main__":
  game()

